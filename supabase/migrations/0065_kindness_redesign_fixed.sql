...
-- Postgres rejects CREATE OR REPLACE when a parameter's DEFAULT is
-- removed, even though the type/arity is unchanged — confirmed by
-- the actual error when this was tried without it. Drop first.
drop function if exists give_kindness(uuid, kindness_category, text);
